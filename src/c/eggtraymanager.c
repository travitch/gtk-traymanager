				  ...
				  EggTrayManagerChild *child)
{
  ...
  utf8_string = XInternAtom (gdk_helper_display(), "UTF8_STRING", False);
  atom = XInternAtom (gdk_helper_display(), "_NET_WM_NAME", False);

  gdk_error_trap_push ();

  result = XGetWindowProperty (gdk_helper_display(),
			       *child_window,
			       atom,
			       0, G_MAXLONG,
			       False, utf8_string,
